;(function () {
    ...
    Guacamole.BlobWriter = function BlobWriter(stream) {
        ...
        var slice = function slice(blob, start, end) {
            ...
            var sliceImplementation = (blob.slice
                || blob.webkitSlice
                || blob.mozSlice).bind(blob);
    ...
    Guacamole.Client = function (tunnel) {
        ...
        this.sendTouchState = function sendTouchState(touchState, applyDisplayScale) {
            ...
            tunnel.sendMessage('touch', touchState.id, Math.floor(x), Math.floor(y), Math.floor(touchState.radiusX), Math.floor(touchState.radiusY), touchState.angle, touchState.force);
        };
        /**
    ...
    Guacamole.Layer = function (width, height) {
        ...
        var resize = function resize(newWidth, newHeight) {
            ...
            if (canvas.width !== canvasWidth || canvas.height !== canvasHeight) {
                ...
                if (oldData)
                    context.drawImage(oldData, 0, 0, oldData.width, oldData.height, 0, 0, oldData.width, oldData.height);
    ...
    Guacamole.StaticHTTPTunnel = function StaticHTTPTunnel(url, crossDomain, extraTunnelHeaders) {
        ...
        this.connect = function connect(data) {
            ...
            fetch(url, {
                ...
                if (!response.ok) {
                    if (tunnel.onerror)
                        tunnel.onerror(new Guacamole.Status(Guacamole.Status.Code.fromHTTPCode(response.status), response.statusText));
                    tunnel.disconnect();
                    return;
                }
